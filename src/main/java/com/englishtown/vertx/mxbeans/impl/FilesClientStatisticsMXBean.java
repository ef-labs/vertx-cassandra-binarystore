 ...
 */
public enum FilesClientStatisticsMXBean implements DefaultPersistorStatisticsMXBean {
    INSTANCE;
    ...
    private FilesClientStatisticsMXBean() {
        final Hashtable<String, String> fileStatsBeanKeys = new Hashtable<>();
        fileStatsBeanKeys.put("subtype", "files");
        fileStatsBeanKeys.put("type", "PersistorStatistics");
        try {
            BeanManager.INSTANCE.registerBean(this, fileStatsBeanKeys);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
